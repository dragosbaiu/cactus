...
class CactusGame:
    ...
    def close_reaction_window(self, player_id: str) -> ActionResult:
        """
        Active player (or server timer) closes the reaction window.
        Moves to special ability if pending, otherwise ends the turn.
        """
        if not self._is_active_player(player_id):
            return ActionResult.fail("Only the active player can close the reaction window.")
        if self.turn_phase != TurnPhase.REACTION:
        ...
        if self.pending_ability:
            ...
            return ActionResult.ok(
                f"Reaction window closed. Resolve {self.pending_ability} ability.",
                {"pending_ability": self.pending_ability}
            )
    ...
    def say_cactus(self, player_id: str) -> ActionResult:
        """
        A player declares Cactus, believing they have the lowest hand sum.
        All other players get one more turn. When it comes back to the
        Cactus player, the round ends.

        Can only be called at the start of a player's turn (DRAW phase).
        """
        ...
        if self.turn_phase != TurnPhase.DRAW:
            return ActionResult.fail("Cactus must be called before drawing.")
        ...
        # Everyone else gets one more turn; when we reach cactus player again → end
        self.cactus_laps_remaining = len(self.players) - 1
        # Advance turn so the NEXT player goes first in the final lap
        self._advance_turn()
        self.turn_phase = TurnPhase.DRAW
    ...
    def _calculate_scores(
        ...
        """
        Calculate points to add for each player this round.

        Rules:
        - Empty hand win → winner gets 3 points, all others get their hand sum
        - Cactus win:
            - If cactus player has the lowest sum → 1 point for them, others get hand sum
            - If cactus player is NOT the lowest → 2 points for them, winner(s) get 1 point,
              others get hand sum
            - Ties between non-cactus players → each tied player gets 2 points,
              cactus player gets 1
        """
        scores: dict[str, int] = {}

        if empty_hand_winner_id:
            for p in self.players:
                if p.player_id == empty_hand_winner_id:
                    scores[p.player_id] = 3
                else:
                    scores[p.player_id] = p.hand_sum
            return scores
        ...
        for p in self.players:
            if p.player_id == self.cactus_player_id:
                # Cactus player: 1 point if they won, 2 if they didn't
                scores[p.player_id] = 1 if cactus_sum == min_sum else 2
            else:
                if p.hand_sum == min_sum and cactus_sum != min_sum:
                    # Tied winner (non-cactus): 1 point
                    scores[p.player_id] = 1
                elif p.hand_sum == min_sum and cactus_sum == min_sum:
                    # Tied with cactus player: 2 points
    ...
    def _determine_next_starter(self, scores: dict[str, int]):
        """
        Decide who starts the next round and store in next_round_starter_id.

        Rules:
        - The player(s) with the lowest score this round won.
        - If one winner → they start next round.
        - If multiple winners tied:
            - The one who earned the most points this round starts
              (higher points = did worse, so actually we want FEWER points —
               wait, lower score = better in Cactus. Among tied winners,
               the one who received MORE points this round starts, because
               that means they were penalised the most among the winners,
               which is the tiebreaker per the rules.)
            - If still tied on points earned → random among them.
        """
        min_score_earned = min(scores.values())
        winners = [
            p for p in self.players
            if scores.get(p.player_id, 0) == min_score_earned
        ]

        if len(winners) == 1:
            self.next_round_starter_id = winners[0].player_id
        else:
            # Among tied winners, the one who got MORE points this round starts
            max_points_this_round = max(scores[w.player_id] for w in winners)
            top_tied = [
                w for w in winners
                if scores[w.player_id] == max_points_this_round
            ]
            # Still tied → random
            self.next_round_starter_id = random.choice(top_tied).player_id
    ...
    def _end_turn(self) -> ActionResult:
        """
        Wrap up the current turn and advance to the next player.
        Handles the Cactus final lap countdown.
        """
        self.turn_phase = TurnPhase.DONE
        ...
        return ActionResult.ok(
            f"Turn ended. It's now {self._active_player.name}'s turn.",
            {
